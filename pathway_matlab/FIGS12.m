...
odeparams.tar_si=0.1; %fast slow si
odeparams.tau_si=1080000; %fast slow si, fast_slow_si_control=360000
odeparams.tar_hepasi=0.3;%fast slow si
odeparams.tau_hepasi=1080000;%fast slow si,fast_slow_si_control=360000
odeparams.Gs=100;%fast slow si
odeparams.ISRI_bar=1.2; % beta-function defect for all pathways, default:1.4
...
odeparams.tar_si=0.1; %fast slow si
odeparams.tau_si=360000; %fast slow si, fast_slow_si_control=360000
odeparams.tar_hepasi=0.3;%fast slow si
odeparams.tau_hepasi=360000;%fast slow si,fast_slow_si_control=360000
odeparams.Gs=100;%fast slow si
odeparams.ISRI_bar=1.2; % beta-function defect for all pathways, default:1.4
...
%%%% end of control simulation

%% plot data
DATA=readtable('slow_si.xlsx');
fs=12;
fn='arial';
...
%%%% read "fast_slow_si_control.xlsx" as DATA2
DATA2=readtable('fast_si.xlsx');
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_si.*6.9444,'r', 'linewidth',lw);
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_HGP,'r', 'linewidth',lw);
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_FG,'r', 'linewidth',lw);

plot(DATA_t./(365*1440),DATA_2hG,'k', 'linewidth',lw);
plot(DATA2_t./(365*1440),DATA2_2hG,'r', 'linewidth',lw);
...
text(1,170,'2hPG', 'fontsize',fs2,'fontname',fn);
text(1,65, 'FPG','fontsize',fs2,'fontname',fn);
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_FI,'r', 'linewidth',lw);

plot(DATA_t./(365*1440),DATA_2hI,'k', 'linewidth',lw);
plot(DATA2_t./(365*1440),DATA2_2hI,'r', 'linewidth',lw);

%xlabel('time (year)','fontsize', fs, 'fontname',fn);
ylabel('{Insulin} ({\mu}U/ml)','fontsize', fs, 'fontname',fn);


text(0,330,'D','fontsize',fpan,'fontweight','bold');
text(1,85,'2hPI', 'fontsize',fs2,'fontname',fn);
text(2,30, 'FPI','fontsize',fs2,'fontname',fn);
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_sigma,'r', 'linewidth',lw);
...
hold('on')
plot(DATA2_t./(365*1440),DATA2_b,'r', 'linewidth',lw);
...
text(0,6600,'F','fontsize',fpan,'fontweight','bold');
axis ([0 10 0 6000])
